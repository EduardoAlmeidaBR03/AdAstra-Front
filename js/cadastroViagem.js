...
const PACOTES_API_URL = "http://localhost:8000/packages/";
...
function setupEventListeners() {
  ...
  elements.tabs.cadastro.addEventListener("click", () => {
    carregarPacotes();
  });
}
...
async function carregarPacotes() {
  try {
    const response = await fetch(PACOTES_API_URL);
    if (!response.ok) {
    ...
    mostrarAlerta("Erro ao carregar lista de pacotes", "danger");
  }
}
...
function renderizarListaViagens(viagens) {
  ...
  viagens.forEach((viagem) => {
    ...
    const titulo =
      palavras.slice(0, 4).join(" ") + (palavras.length > 4 ? "..." : "");
    ...
    card.innerHTML = `
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <p class="card-text">
                        <strong>Data de Partida:</strong> ${new Date(
                          viagem.data_partida
                        ).toLocaleString()}<br>
                        <strong>Duração:</strong> ${viagem.duracao_horas}h<br>
                        <strong>Status:</strong> ${viagem.status}
                    </p>
                    <button class="btn btn-primary btn-sm" onclick="carregarDetalhesViagem('${
                      viagem.id
                    }')">
                        Ver Detalhes
                    </button>
...
function renderizarDetalhesViagem(viagem) {
  ...
  elements.detalhes.pacote.textContent = viagem.pacote.nome;
  elements.detalhes.dataPartida.textContent = new Date(
    viagem.data_partida
  ).toLocaleString();
